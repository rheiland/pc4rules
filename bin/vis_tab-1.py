    ...
    #    print('time=' + root.find(".//current_time").text)
        ...
        if (self.fix_cmap_flag):
            ...
            try:
                ...
                # substrate_plot = self.ax0.contour(xgrid,ygrid,zvals, [0.002, 0.004, 0.006, 0.008, 0.01], cmap=cbar_name) #, vmin=Z.min(), vmax=Z.max())
                try:
                    substrate_plot = self.ax0.contour(xgrid,ygrid,zvals) #, vmin=Z.min(), vmax=Z.max())
                except:
